...
# ✅ Always point to the DB in the same folder as this file (server/app.db)
...
class ClearSession(Resource):
    ...
    # ✅ Tests call GET /clear, so support it too
    def get(self):
        session['page_views'] = None
        session['user_id'] = None
        return {}, 204
...
class IndexArticle(Resource):
    def get(self):
        articles = [ArticleSchema().dump(article)
...
class MemberOnlyArticle(Resource):
    def get(self, id):
        ...
        # ✅ Test expects access by ID once logged in
        article = Article.query.filter(Article.id == id).first()
        article_json = ArticleSchema().dump(article)
...
api.add_resource(MemberOnlyArticle,
                 '/members_only_articles/<int:id>', endpoint='member_article')
